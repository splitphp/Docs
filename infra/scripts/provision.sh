...
# your IP; DNS A records `mcp.splitphp.org` AND `api.splitphp.org` → the instance's public (Elastic) IP.
#
...
# Local REST API plugin (public third-party release → downloadable directly).
mkdir -p "$REST_DIR"
if [ ! -f "$REST_DIR/main.js" ]; then
  REST_TAG="$(curl -s https://api.github.com/repos/coddingtonbear/obsidian-local-rest-api/releases/latest | grep -oP '"tag_name":\s*"\K[^"]+')"
  REST_BASE="https://github.com/coddingtonbear/obsidian-local-rest-api/releases/download/${REST_TAG}"
  curl -fL --retry 3 -o "$REST_DIR/main.js"      "$REST_BASE/main.js"
  curl -fL --retry 3 -o "$REST_DIR/manifest.json" "$REST_BASE/manifest.json"
  curl -fL --retry 3 -o "$REST_DIR/styles.css"    "$REST_BASE/styles.css" 2>/dev/null || true
fi

echo "== 7/10 keys + data.json (MCP: readOnlyMode; REST: loopback HTTP) =="
if [ ! -f /etc/mcp.key ]; then
  umask 077
  openssl rand -base64 36 | tr -d '\n/+=' | cut -c1-48 | sudo tee /etc/mcp.key >/dev/null
  sudo chmod 600 /etc/mcp.key
fi
if [ ! -f /etc/rest.key ]; then
  umask 077
  openssl rand -base64 36 | tr -d '\n/+=' | cut -c1-48 | sudo tee /etc/rest.key >/dev/null
  sudo chmod 600 /etc/rest.key
fi
umask 077
...
PY
# The Local REST API plugin is write-capable; write access is fenced off at Caddy (only GET /vault/*).
# Here it runs loopback HTTP (Caddy fronts TLS) with auth on; the secure 27124 port stays loopback too.
KEY="$(sudo cat /etc/rest.key)" python3 - "$REST_DIR/data.json" <<'PY'
import json, os, sys
json.dump({
  "apiKey": os.environ["KEY"],
  "enableInsecureServer": True, "insecurePort": 27123,
  "port": 27124, "bindingHost": "127.0.0.1",
}, open(sys.argv[1], "w"), ensure_ascii=False, indent=2)
PY
...
{ printf 'MCP_KEY=%s\n' "$(sudo cat /etc/mcp.key)"
  printf 'REST_KEY=%s\n' "$(sudo cat /etc/rest.key)"; } | sudo tee /etc/caddy/mcp.env >/dev/null
...
echo
echo "Done. Verify (after DNS/SG are ready and the certs are issued):"
echo "  curl -s -o /dev/null -w '%{http_code}\\n' https://$DOMAIN/mcp          # expect 400"
echo "  curl -s -o /dev/null -w '%{http_code}\\n' https://$API_DOMAIN/vault/   # expect 200"
echo "  curl -s -o /dev/null -w '%{http_code}\\n' https://$API_DOMAIN/         # expect 403"
